---
title: 'Analysis pipline - Functionally Enigmatic Genes in Cancer: Are  We Still Looking
  Under the Lamppost for the Lost Keys?'
author: Alexandra Maertens, Vy Tran, Mikhail Maertens, Andre Kleensang, Thomas Luechtefeld,
  Thomas Hartung, and Channing Paller
date: "August 13, 2019"
output:
...
setwd("C:/Users/vytran/OneDrive - JHSPH/Functionally Enigmatic Genes in Cancer/Supplemental data/Rmarkdown and Raw data")
getwd()
```
...
# Import PubMed ID data:
load(file = "PubMed.ID.for.all.TCGA.genes.Rdata")

#Load prognostic unfavorable gene data:
...
```{r}
# Create pie charts for figures 1 b-e:
...
packages = c("dplyr", "tidyr","WGCNA")

# Check if required packages are installed:
sapply(X = packages, FUN = require, character.only = TRUE, quietly=TRUE)
```
PRAD, COAD, and GBMLGG RNAseq data sets were downloaded as illuminahiseq_rnaseqv2-RSEM_genes_normalized (MD5) data from GDAC Firehose https://gdac.broadinstitute.org/. Data sets were saved in my computer as an Rdata file. Data preprocessing:
```{r}
# Important setting:
...
save(MEs_PRAD, moduleLabels_PRAD, moduleColors_PRAD, geneTree_PRAD,
     file = "PRADnetwork_modulecolor_and_label.RData")
```
